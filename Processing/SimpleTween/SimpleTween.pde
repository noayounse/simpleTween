...
void setup() {
  ...
  pvst = new PVSTween(2.4, 1, new PVector(33, 20), new PVector(width/2, height/2));
  ...
  mst = new MSTween(5, "mySimpleTestMethod", testDot);
...
void keyReleased() {
  ...
  if (key == 'e') pvst.setDelay(3);
  if (key == 'f') pvst.setDelay(1);

  if (key == 'c') pvst.clearTargets();
